...
{
public:
    KtSptDijkstra(const GRAPH& g, unsigned v0) : KtSpt<GRAPH, WEIGHTOR>(g, v0) {
...
{
public:
	KtSptPfs(const GRAPH& g, unsigned v0) : KtSpt<GRAPH, WEIGHTOR>(g, v0) {
...
{
public:
    KtAllSptFloyd(const GRAPH& g) : KtAllSpt<GRAPH, WEIGHTOR>(g) {
...
{
public:
	KtAllSptDfs(const GRAPH& g) : KtAllSpt<GRAPH, WEIGHTOR>(g), done_(g.order(), false) {
		assert(!g.hasLoop());
